...
import amg_solver_opt

def poisson(shape, format='csr'):
    nx, ny = shape
    values, colind, rowptr, rows, cols = amg_solver_opt.poisson(nx, ny)
...
class SmoothedAggregationSolver:
    ...
    def solve(self, b, x0=None, tol=1e-8, callback=None):
        b = b.tolist()
        x = amg_solver_opt.amg_vcycle(self.values, self.colind, self.rowptr,
                                      self.rows, self.cols, b, tol, 10,
                                      self.nx, self.ny)
        x = np.array(x)
        if callback:
            callback(x)
    ...
    def smoothed_aggregation_solver(A):
        nx = ny = int(np.sqrt(A.shape[0]))
        return SmoothedAggregationSolver(A, nx, ny)
...
class RugeStubenSolver:
    ...
    def solve(self, b, x0=None, tol=1e-8, callback=None):
        b = b.tolist()
        x = amg_solver_opt.cg_solve(self.values, self.colind, self.rowptr,
